...
use sxd_document_no_unsafe::{Package, QName, as_qname, as_str, local_name, text_interned};
...
fn intent_rules<'m>(rules: &'static std::thread::LocalKey<RefCell<SpeechRules>>, doc: Document<'m>, mathml: Element, nav_node_id: &'m str) -> Result<Element<'m>> {
    rules.with(|rules| {
        ...
        let original_intent = mathml.attribute_value_interned("intent").map(|g| as_str!(g).to_string());
        if should_set_literal_intent {
            if let Some(ref intent) = original_intent {
                let intent = if intent.contains('(') {intent.replace('(', ":literal(")} else {intent.clone() + ":literal"};
        ...
        if should_set_literal_intent {
            if let Some(ref original_intent) = original_intent {
...
fn speak_rules(rules: &'static std::thread::LocalKey<RefCell<SpeechRules>>, mathml: Element, nav_node_id: &str, nav_node_offset: usize) -> Result<String> {
    ...
    fn nestable_speak_rules<'c, 's:'c, 'm:'c>(rules_with_context: &mut SpeechRulesWithContext<'c, 's, 'm>, mathml: Element<'c>) -> Result<String> {
        ...
        if !rules_with_context.nav_node_id.is_empty() {
            ...
            let _ai_intentattr = mathml.attribute_value_interned("data-intent-property");
            let intent_attr = _ai_intentattr.as_deref().unwrap_or("");
...
impl Intent {
    ...
    fn replace<'c, 's:'c, 'm: 'c, T:TreeOrString<'c, 'm, T>>(&self, rules_with_context: &mut SpeechRulesWithContext<'c, 's,'m>, mathml: Element<'c>) -> Result<T> {
        ...
        if mathml.parent().is_some() && mathml.parent().unwrap().element().is_some() &&
           result.attribute_value_interned("id") == crate::canonicalize::get_parent(mathml).attribute_value_interned("id") {
    ...
    fn replace<'c, 's:'c, 'm:'c, T:TreeOrString<'c, 'm, T>>(&self, rules_with_context: &mut SpeechRulesWithContext<'c, 's,'m>, mathml: Element<'c>) -> Result<T> {
        if self.xpath.rc.string.starts_with('@') {
            ...
            let id = match xpath_value {
                ...
                Value::Nodeset(nodes) => {
                    if nodes.size() == 1 {
                        nodes.document_order_first().unwrap().attribute().map(|attr| attr.value_interned().to_string())
                    } else {
                        None
                    }
                },
...
impl<'c, 's:'c, 'r, 'm:'c> SpeechRulesWithContext<'c, 's,'m> {
    ...
    pub fn match_pattern<T:TreeOrString<'c, 'm, T>>(&'r mut self, mathml: Element<'c>) -> Result<T> {
        ...
        if let Some(rule_vector) = rules.get(as_str!(tag_name)) &&
           let Some(result) = self.find_match(rule_vector, mathml)? {
                return Ok(result);      // found a match
            }
        ...
        for pattern in rule_vector {
            ...
            if pattern.is_match(&self.context_stack.base, mathml)
                ...
                let result = if self.nav_node_offset > 0 &&
                            mathml.attribute_value_interned("id").map_or(false, |g| self.nav_node_id == &*g) && is_leaf(mathml) {
                ...
                return match result {
                    Ok(s) => {
                        ...
                        if self.nav_node_id.is_empty() {
                            ...
                            if mathml.attribute_value_interned("id").map_or(false, |g| self.nav_node_id == &*g) {debug!("Matched pattern name/tag: {}/{}", pattern.pattern_name, pattern.tag_name)};
    ...
    fn nav_node_adjust<T:TreeOrString<'c, 'm, T>>(&self, speech: T, mathml: Element<'c>) -> T {
      if let Some(id) = mathml.attribute_value_interned("id") &&
         self.nav_node_id == as_str!(id) {
        let _ai_offset = mathml.attribute_value_interned(crate::navigate::ID_OFFSET);
        let offset = _ai_offset.as_deref().unwrap_or("0");
        debug!("nav_node_adjust: id/name='{}/{}' offset?='{}'", as_str!(id), name(mathml),
        ...
        if is_leaf(mathml) || self.nav_node_offset.to_string().as_str() == offset {
          if self.speech_rules.name == RulesFor::Braille {
            ...
            debug!("nav_node_adjust: id='{}' offset='{}/{}'", as_str!(id), self.nav_node_offset, offset);
        ...
        for node in nodes {
            let matched = match node {
                ...
                Node::Text(t) =>  {
                    ...
                    leaf.set_text(as_str!(text_interned!(t)));
                    leaf
                },
                Node::Attribute(attr) => {
    ...
    fn replace_nodes_string(&'r mut self, nodes: Vec<Node<'c>>, mathml: Element<'c>) -> Result<String> {
        ...
        for node in nodes {
            ...
            let matched = match node {
                ...
                Node::Text(t) =>  self.replace_chars(as_str!(text_interned!(t)), mathml)?,
